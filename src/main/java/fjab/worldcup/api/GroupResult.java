...
public final class GroupResult {
	...
	*  @param matrix Bidimensional array of Integers to be checked
	 ...
	 */
	private boolean checkElementsDistribution(Integer[][] originalCombination) {				
	...
	private void sortArray(Integer[][] matrix) {
		...
		Integer scarcestElement = IntegerMatrix.findLeastFrequentElement(replaceMinus1By1(matrix), true);
		int index;
		if(scarcestElement==null){
			...
			scarcestElement = IntegerMatrix.findLeastFrequentElement(replaceMinus1By1(matrix), false);
		}
		else{
